...
@mcp.tool()
def get_web_ui_link(include_login_link: bool = True) -> dict[str, Any]:
    ...
    if include_login_link:
        if automation_token:
            result["login_link_available"] = False
            result["login_link_reason"] = "automation tokens cannot create web login links"
        else:
